...
class BasePubkeyAlgorithm(ABC):
	...
	@lru_cache
	def import_ffi_module(self, variant: Variant) -> ModuleType:
	...
	def __init__(self):
		try:
			self._lib = self.import_ffi_module(Variant.AVX2)
			self.variant = Variant.AVX2
		except ModuleNotFoundError:
			try:
				self._lib = self.import_ffi_module(Variant.CLEAN)
	...
	@staticmethod
	def validate(data: Any, exp_size: int, param_name: str) -> None:
